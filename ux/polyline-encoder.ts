...
function updateEncoder() {
    ...
    let geom = new ol.geom.LineString(<Array<number[]>>JSON.parse(input.value));
...
function updateCanvas(canvas_id: string, features_id: string) {
    ...
    let geom = new ol.geom.LineString(<Array<number[]>>JSON.parse((<HTMLTextAreaElement>$(features_id)[0]).value));
...
export function run() {
    ...
    $(".input textarea")
        ...
        .change(args => {
            ...
            let coords = <number[][]>JSON.parse(`${input.value}`);
