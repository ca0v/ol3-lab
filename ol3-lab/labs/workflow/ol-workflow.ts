...
export function run() {
	...
	select.on("select", (args: Event & { selected: ol.Feature[]; deselected: ol.Feature[] }) => {
	...
	if (true) {
		...
		maplet.Controls.Controls.forEach(l => {
			l.Commands && importCommands(<any>l.Commands.Commands, eventHash);
		});
...
function importEvents(events: typeof MapletData.data.Events.Events, eventHash: Map<string, WorkFlowItem>) {
	events.forEach(event => {
		...
		event.name.split(",").forEach(eventName => {
			...
			let eventOption: { value: string } =
				event.Options && event.Options.Values && (<any>event.Options.Values).filter(v => v.id === "event")[0];
...
function importCommands(events: typeof MapletData.data.Commands.Commands, eventHash: Map<string, WorkFlowItem>) {
	events.forEach(event => {
		...
		let eventOption: { value: string } =
			event.Options && event.Options.Values && event.Options.Values.filter(v => v.id === "event")[0];
