...
export function run() {
	...
	MapMaker.run({
				...
				let interactions = map
					...
					.filter(
						i =>
							...
							i instanceof ol.interaction.Translate
					);
		...
		map.addControl(
			Draw.create({ geometryType: "MultiPolygon", label: "▧", position: "right-4 top", layers: [layer] })
		);
		map.addControl(
			Draw.create({ geometryType: "MultiLineString", label: "▬", position: "right-2 top", layers: [layer] })
		);
