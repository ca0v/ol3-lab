...
const ol = {
  ...
  layer: olLayer,
  Map: olMap,
  source: { Tile: TileSource, OSM: OSMSource, BingMaps: BingMapsSource },
};
...
export function run() {
        ...
        source:
          options.basemap !== "bing"
            ...
            : new ol.source.BingMaps({
                key:
                  "AuPHWkNxvxVAL_8Z4G8Pcq_eOKGm5eITH_cJMNAyYoIC1S_29_HhE893YrUUbIGl",
                imagerySet: "Aerial",
              }),
      }),
    ],
  ...
  function showDefect() {
    map.getView().fit([-82.4, 34.85, -82.39, 34.851], {
      size: map.getSize(),
    });
  }

  function showSolution() {
    fit(map, [-82.4, 34.85, -82.39, 34.851]);
  }
  ...
  function fit(map: olMap, extent: [number, number, number, number]) {
