...
let openFile = function(event: Event) {
	...
	reader.onload = () => {
		...
		(<HTMLTextAreaElement>textarea[0]).value = reader.result as string;
		textarea.change();
	};
...
let pasteHandler = (evt: ClipboardEvent & { originalEvent: any }) => {
	...
	if (blob) {
		...
		reader.onload = readerEvent => {
			$(".image-data-input")
				.val((<any>readerEvent.target).result)
				.change();
		};
...
export function run() {
	...
	$(".image-data-input").on("paste", <any>pasteHandler);

	$(".image-file").change(<any>openFile);
}
