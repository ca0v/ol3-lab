...
define("ux/polyline-encoder", ["require", "exports", "jquery", "openlayers", "google-polyline"], function (require, exports, $, ol, PolylineEncoder) {
    ...
    function updateEncoder(features_id) {
        var geom = new ol.geom.LineString(JSON.parse(document.getElementById(features_id).value));
    ...
    function run() {
        $(css).appendTo("head");
        $(ux).appendTo(".map");
        sample_input.forEach(function (p) {
            for (var i = 0; i < p.length; i++)
                p[i] = 0.0001 * Math.round(p[i] * 10000);
        });
        $("#polyline-output").change(function (args) {
            updateCanvas("after", "polyline-output");
            updateEncoder("polyline-output");
        });
        $("#polyline-input")
            .text(JSON.stringify(sample_input))
            .change(function (args) {
            var coords = JSON.parse("" + args.target.value);
            var geom = new ol.geom.LineString(coords);
            geom = geom.simplify(0.00001);
            $("#polyline-output").text(JSON.stringify(geom.getCoordinates())).change();
            updateCanvas("before", "polyline-input");
        })
            .change();
    }
    exports.run = run;
